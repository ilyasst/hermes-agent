...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        # duration is caught and killed.  Per-job "timeout" field takes
        # precedence, then HERMES_CRON_TIMEOUT env var, then 10-minute default.
        # 0 = unlimited.
        #
        ...
        if _raw_cron_timeout:
            try:
                _default_timeout = float(_raw_cron_timeout)
            ...
            _default_timeout = 600.0
        try:
            _cron_timeout = float(job.get("timeout", _default_timeout))
        except (ValueError, TypeError):
            logger.warning(
                "Invalid per-job timeout=%r; using default %ss",
                job.get("timeout"), _default_timeout,
            )
            _cron_timeout = _default_timeout
