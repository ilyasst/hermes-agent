...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        # The request body is fixture-controlled, so pin the other catalog
        # input too.  Otherwise the expected entries depend on whichever
        # remotely-curated manifest happens to be served during this test.
        with (
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        # Keep the curated input deterministic for the same reason as the
        # pricing-tag test above: this test exercises permissive handling of
        # missing capability metadata, not a remote manifest's membership.
        with (
            patch("hermes_cli.model_catalog.get_curated_openrouter_models", return_value=None),
            patch("hermes_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
