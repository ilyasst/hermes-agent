...
_truncate_oversized_read_cached: bool | None = None


def _truncate_oversized_read_enabled() -> bool:
    """[LOCAL Patch J] When a read exceeds the char limit, return the
    truncated head plus a concrete continuation directive instead of a hard
    error. A weak local model can act on "here is the head, call again with
    offset=N" but often cannot recover from a bare error, looping on the
    identical failing call. Default True; disable via config.yaml:
        local_patches:
    ...
    try:
        ...
        _truncate_oversized_read_cached = bool(
            lp.get("patch_j_truncate_oversized_read", True)
        )
        return _truncate_oversized_read_cached
    except Exception:
        pass
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        if content_len > max_chars:
            ...
            # [LOCAL Patch J] Truncate-and-continue: hand back the head trimmed
            # to a line boundary plus a concrete next call. This is a SUCCESS
            # shape (no "error" key) so it is not classified as a tool failure
            # and each continuation differs — loops cannot form.
...
# (e.g., Telegram / constrained local-model nodes) can opt out of read_file
# while keeping write/patch/search. Motivation: the agent over-uses read_file
# for trivial queries, loading whole files into context when `wc -l` / `grep`
# via bash would suffice. Removing read_file from a node's toolset forces bash
# for file inspection, naturally favoring compound commands. Config-side: the
# "file_read" configurable toolset (toolsets.py / hermes_cli.tools_config) must
# be present in a node's platform_toolsets list to keep read_file; constrained
# nodes omit it. NOTE: max_result_size_chars kept at the harmonized
# a5e466719 value (100_000), NOT the old fork spec-commit's float('inf').
