...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Recall past conversations via FTS5 search across session transcripts. "
        "No query → browse recent sessions (instant, no LLM). With query → "
        "LLM-summarize matching sessions. Use when user says 'last time', "
        "'remember when', or references prior work not in current context. "
        "Syntax: OR between keywords for broad recall (FTS5 defaults to AND "
        "which misses partial matches), quotes for phrases, NOT for exclusion, "
        "prefix* for stems."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "Keywords/phrases/boolean. Omit to browse recent."},
            "role_filter": {"type": "string", "description": "Comma-separated roles, e.g. 'user,assistant'."},
